...
service_name=${serviceName:-${container_name}} # name of servise
backup_path=${backupPath:-'Error!'}
count_save=${countSave:-"7"} # How many archives must be saved
host_database=${hostDB:-$container_name} # Host where work docker container with database
...
function createDump () {
    ...
    case ${db_type} in
      ...
      'redis') # Backup redis
         ...
         format_option_tar='z'
       ;;
       'postgressql') # Backup postgrssql
         port=${hostPort:-"5432"} # Port where work container
         DB_USERNAME=${db_username} # variable with username for postgressql
         DB_PASSWORD=${db_password} # variable with password for postgressql

         PGPASSWORD="${db_password}" pg_dumpall -U '${db_username}' -h ${host_database} -p ${port} | gzip > ${backup_path}${db_type}.sql.gz
      ;;

      'mongo') # Backup mongo
         port=${hostPort:-"27017"} # Port where work container
         DB_USERNAME=${db_username} # variable with username for mongo
         DB_PASSWORD=${db_password} # variable with password for mongo

         # Command to cretae docker container what connect to mongo container and move backup file to local system.
         mongodump --username='${db_username}' --password='${db_password}' --authenticationDatabase=admin --gzip --out ${backup_path}${db_type} --host ${host_database}:${port}
      ;;

      'mysql') # Backup mysql
         port=${hostPort:-"3306"} # Port where work container
         DB_USERNAME=${db_username} # username mysql
         DB_PASSWORD=${db_password} # password mysql
